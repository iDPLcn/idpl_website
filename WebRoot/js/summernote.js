        ...
        var insertImages = function ($editable, files) {
            ...
            if (callbacks.onImageUpload) {
                ...
                $.each(files, function (idx, file) {
                     ...
                     */
                    $.ajaxFileUpload({
                        url: "upload.action",// 文件上传服务器请求Action
                        secureuri: true,// 安全提交，默认为false
                        fileElementId: "upload",// 文件类型的id
                        dataType: "json",
                        success: function(data, status){// 服务器响应成功
                            if (status == "success") {
                                var returnURL = $.parseJSON(data).urldata;
//                                var returnURL = data.firstElementChild.innerText;
//                                alert(returnURL);
                                editor.insertImage($editable, returnURL, filename);
                            }
                            else {
                                if (callbacks.onImageUploadError) {
                                    callbacks.onImageUploadError();
                        ...
                        error: function(data, status){// 服务器响应失败
                            if (callbacks.onImageUploadError) {
                                callbacks.onImageUploadError();
                            }
                        }
        ...
        var tplDialogs = function (lang, options) {
            var tplImageDialog = function () {
                var body =
                    ...
                    '<input class="note-image-input" type="file" name="files" id="upload" accept="image/*" />' +
                    '</div>' +
                    '<h5>' + lang.image.url + '</h5>' +
                    '<input class="note-image-url form-control span12" id="file_url" type="text" />';
        ...
        summernote: function (options) {
            ...
            this.each(function (idx, elHolder) {
                ...
                if (dom.isTextarea($holder[0])) {
                    ...
                    // auto filling textarea when click the button with class "btn-submit"
                    $holder.closest('form').find('.btn-submit').click(function () {
                        $holder.val($holder.code());
                    });
                }
            });
